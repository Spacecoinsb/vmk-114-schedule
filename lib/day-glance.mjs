...
export function dayGlance(schedule,date,now,subgroups={}){
  ...
  if(current){
    ...
    return {kind:'now',title:`Пара закончится через ${duration(minutes(current.end)-clock)}`,
  ...
  if(upcoming){
    ...
    return {kind:first?'before':'break',title:`${first?'Первая':'Следующая'} пара начнётся через ${duration(minutes(upcoming.start)-clock)}`,
