...
export const WING = x => x < 0.5 ? 'Север' : 'Юг';
const wingText = x => x < 0.5 ? 'северное крыло' : 'южное крыло';
...
export const POINTS = FLOORS.flatMap(f => [
  ...f.rooms.map(r => ({key:`${f.floor}:${r.id}`, floor:f.floor, id:r.id, kind:'room', name:/^(П|МЗ)-/.test(r.id) ? r.id : `Аудитория ${r.id}`, x:r.x, y:r.y, note:r.note})),
  ...f.places.map(p => ({key:`${f.floor}:${p.id}`, floor:f.floor, id:p.id, kind:p.kind, name:p.name, x:p.x, y:p.y, note:p.note})),
]);
...
function graph(floorNo) {
  ...
  const f = floorOf(floorNo), k = f.aspect;
  const nodes = [], edges = new Map();
  const add = (x, y) => { const found = nodes.findIndex(n => Math.hypot((n.x-x)*k, n.y-y) < 1e-4); if (found >= 0) return found; nodes.push({x, y}); return nodes.length-1; };
