...
try {
  ...
  assert(await page.getByRole('link',{name:'Сайт ВМК'}).isVisible());
  ...
  assert.equal(await page.locator('.lesson.lecture .type').count(),await page.locator('.lesson.lecture').count());
  ...
  await page.getByRole('button',{name:'Ближайшие',exact:true}).click();
  await page.locator('h1',{hasText:'26 сентября'}).waitFor();
